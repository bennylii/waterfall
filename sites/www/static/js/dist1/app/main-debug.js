                ...
                function initialize() {
                    ...
                    for (var i = 0; i < noOfBlocks; i++) {
                        ...
                        allElements.slice(startIndex, endIndex).each(function(index, el) {
                            if (!copy) wrapper.append(el); else wrapper.append($(el).clone(true));
                        });
...
(function($) {
    ...
    Wookmark = function(options) {
        ...
        Wookmark.prototype.layout = function() {
            ...
            var columnWidth = this.getItemWidth() + this.offset, containerWidth = this.container.width(), columns = Math.floor((containerWidth + this.offset) / columnWidth), offset = 0, bottom = 0;
     ...
     */
        Wookmark.prototype.layoutFull = function(columnWidth, columns, offset) {
            ...
            // Loop over items.
            var item, top, left, i = 0, k = 0, length = this.handler.length, shortest = null, shortestIndex = null, bottom = 0, itemCSS = {
                position: "absolute"
            }, sideOffset;
            for (;i < length; i++) {
     ...
     */
        Wookmark.prototype.layoutColumns = function(columnWidth, offset) {
            var heights = [], length = this.columns.length, i = 0, column, k = 0, kLength, item, bottom = 0, itemCSS, sideOffset;
...
function initSWFUpload() {
    new SWFUpload({
        ...
        // Button Settings
        button_image_url: "/static/images/upload_button.png",
        ...
        custom_settings: {
            uploading_wrap: "#uploadingWrap",
            upreview_list: "#upreviewList",
            upload_target: "uprocessWrap",
            list_wrap: "#tiles"
        },
        // Debug Settings
        debug: false
...
function uploadSuccess(file, serverData) {
    var rsp = $.parseJSON(serverData);
    try {
        var progress = new FileProgress(file, this.customSettings.upload_target);
        if (rsp.status === 1) {
            addImage("/uploads/" + rsp.data.thumbUrl + "_230", rsp.data.title);
            progress.setStatus("预览图片");
            progress.toggleCancel(false);
        } else {
            //addImage("images/error.gif");
            progress.setStatus(rsp.statusText);
            progress.toggleCancel(false);
        }
    } catch (ex) {
        this.debug(ex);
...
function uploadComplete(file) {
    try {
        ...
        if (this.getStats().files_queued > 0) {
            ...
            progress.setStatus("所有照片已上传完毕,5秒后刷新显示.");
            progress.toggleCancel(false);
            var uploadingWrap = this.customSettings.uploading_wrap;
            var list_wrap = this.customSettings.list_wrap;
            setTimeout(function() {
                $(uploadingWrap).hide("slow", function() {
                    location.reload();
                });
            }, 5e3);
        }
...
function addImage(src, title) {
    var li = document.createElement("li"), newImg = document.createElement("img"), p = document.createElement("p");
    p.innerHTML = title;
    newImg.style.width = "100px";
    li.appendChild(newImg);
    li.appendChild(p);
    $("#upreviewList").append(li);
    //    console.log(li);
    if (newImg.filters) {
        try {
...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper) {
        ...
        this.fileProgressWrapper.className = "uprogressWrapper";
        this.fileProgressWrapper.id = this.fileProgressID;
        this.fileProgressElement = document.createElement("div");
        this.fileProgressElement.className = "uprogressContainer";
        var progressCancel = document.createElement("a");
        progressCancel.className = "uprogressCancel";
        progressCancel.href = "#";
        progressCancel.style.visibility = "hidden";
        progressCancel.appendChild(document.createTextNode(" "));
        var progressText = document.createElement("div");
        progressText.className = "uprogressName";
        progressText.appendChild(document.createTextNode(file.name));
        var progressBar = document.createElement("div");
        progressBar.className = "uprogressBarInProgress";
        var progressStatus = document.createElement("div");
        progressStatus.className = "uprogressBarStatus";
...
FileProgress.prototype.setProgress = function(percentage) {
    this.fileProgressElement.className = "uprogressContainer green";
    this.fileProgressElement.childNodes[3].className = "uprogressBarInProgress";
    this.fileProgressElement.childNodes[3].style.width = percentage + "%";
};

FileProgress.prototype.setComplete = function() {
    this.fileProgressElement.className = "uprogressContainer blue";
    this.fileProgressElement.childNodes[3].className = "uprogressBarComplete";
    this.fileProgressElement.childNodes[3].style.width = "";
};

FileProgress.prototype.setError = function() {
    this.fileProgressElement.className = "uprogressContainer red";
    this.fileProgressElement.childNodes[3].className = "uprogressBarError";
    this.fileProgressElement.childNodes[3].style.width = "";
};
    ...
    function initSwitchThumbSize(waterfall) {
        $.tabs({
            ...
            click: function(index, instance) {
                ...
                waterfall.reset({
                    size: parseInt($(this).attr("tab-key")),
                    page: 1,
                    isEnd: false
                });
            }
